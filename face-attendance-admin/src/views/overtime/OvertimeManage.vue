    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>加班管理</span>
          <el-button type="primary" @click="handleAdd">申请加班</el-button>
        </div>
      </template>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="hours" label="加班时长(小时)" width="120" />
        ...
        <el-table-column prop="compensationType" label="补偿方式" width="100">
          <template #default="{ row }">
            <el-tag :type="row.compensationType === 0 ? 'primary' : 'success'">
              {{ row.compensationType === 0 ? '调休' : '加班费' }}
            </el-tag>
          </template>
        </el-table-column>
...
import { getOvertimeList, submitOvertime, approveOvertime, rejectOvertime } from '@/api'
...
const handleAdd = () => {
  Object.assign(form, { userId: 1, overtimeDate: '', hours: 1, compensationType: 0, reason: '' })
...
const handleApprove = async (row) => {
  try {
    await ElMessageBox.confirm('确定通过该加班申请？', '提示', { type: 'info' })
    await approveOvertime(row.id, { approverId: 1, comment: '审批通过' })
...
const handleReject = async (row) => {
  try {
    await ElMessageBox.confirm('确定拒绝该加班申请？', '提示', { type: 'warning' })
    await rejectOvertime(row.id, { approverId: 1, comment: '不符合加班条件' })
